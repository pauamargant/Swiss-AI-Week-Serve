...
async def speech_to_text(audio_file: UploadFile = File(...)):
    ...
    try:
        ...
        result = stt_service.stt(audio_data)
        return {"text": result}
...
async def text_to_speech(request: TTSRequest):
    ...
    try:
        ...
        # Call TTS service forwarding optional parameters if supported
        # We defensively attempt to pass language/voice if the implementation accepts them.
        try:
            audio_data = tts_service.tts(text=request.text.strip(), language=request.language, voice=request.voice)
        except TypeError:
            # Backward compatibility if older signature without language/voice
            audio_data = tts_service.tts(request.text.strip())

        if not audio_data:
            logger.error("TTS service returned empty/None audio_data")
            raise HTTPException(status_code=500, detail="TTS service failed to generate audio")

        # If it's a path (string), load bytes
        if isinstance(audio_data, str):
            logger.debug(f"TTS returned string of length {len(audio_data)}. Attempting to interpret as file path or base64.")
            # File path case
            if os.path.exists(audio_data):
                try:
                    ...
                    logger.error(f"Failed reading audio file path returned by TTS service: {fe}")
                    raise HTTPException(status_code=500, detail="Failed to load audio file produced by TTS service")
                finally:
                    try:
                ...
                try:
                    ...
                    logger.error(f"String from TTS is not a valid base64 audio payload: {be}")
                    raise HTTPException(status_code=500, detail="TTS service returned invalid string (not file path or base64)")

        if not isinstance(audio_data, bytes):
            logger.error(f"TTS service returned non-bytes object of type {type(audio_data)}")
            raise HTTPException(status_code=500, detail=f"TTS service returned invalid audio format: {type(audio_data)}")
        if len(audio_data) == 0:
            logger.error("TTS service returned zero-length bytes")
            raise HTTPException(status_code=500, detail="TTS service returned empty audio bytes")
        logger.debug(f"Final audio bytes length: {len(audio_data)}")

        # ElevenLabs (and most modern cloud TTS providers) produce MP3 here. Use correct media type.
